...
#include <sne/server/s2s/ServerInfo.h>
...
/**
 * @class DeferrableProxyGameDatabaseCallback
 */
class DeferrableProxyGameDatabaseCallback
{
public:
    virtual ~DeferrableProxyGameDatabaseCallback() {}

    virtual sne::base::Future::Ref getFuture(
        sne::database::proxy::RequestId requestId) = 0;
};
...
/**
 * @class DeferrableProxyGameDatabase
 *
 * Database Proxy Server를 이용하는 GameDatabase 클래스.
 * - EventMachine의 deferrable 개념을 차용
 */
class DeferrableProxyGameDatabase :
    public ProxyGameDatabase,
    private DeferrableProxyGameDatabaseCallback
{
    typedef sne::core::HashMap<sne::database::proxy::RequestId,
        sne::base::Future::Ref> RequestFutureMap;

public:
    DeferrableProxyGameDatabase(sne::base::IoContextTask& ioServiceTask);
...
private:
    ...
    virtual void registGameMailCallback(GameMailCallback& gameMailCallback);
    virtual void registDBQueryPlayerCallback(DBQueryPlayerCallback& playerCallback);
    virtual void registBuddyCallback(CommunityBuddyCallback& buddyCallback);
...
private:
    // = DeferrableProxyGameDatabaseCallback overriding
    virtual sne::base::Future::Ref getFuture(
        sne::database::proxy::RequestId requestId) {
        return popFuture(requestId);
    }

private:
    bool initDatabaseProxy(const sne::server::ServerInfo& serverInfo,
        msec_t connectionTimeout);

    void initRpc();

    sne::database::proxy::RequestId pushFuture(
        sne::base::Future::Ref future);
    sne::base::Future::Ref popFuture(
        sne::database::proxy::RequestId requestId);

private:
    sne::base::IoContextTask& ioServiceTask_;

    sne::database::proxy::RequestId currentRequestId_;
    RequestFutureMap requestFutureMap_;

    std::unique_ptr<sne::database::proxy::DatabaseClientSideProxy> databaseProxy_;
    DatabaseProxyClientSideRpcExtension* rpcExtension_;
